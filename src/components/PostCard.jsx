...
const PostCard = ({ title, content, time, rank, id, getFeed, activeLike2, picture, userName }) => {
  ...
  return (
    <div className="flex bg-white shadow-lg rounded-lg md:max-w-xl mx-auto w-full overflow-hidden">
      <div className="flex items-start px-4 py-6 w-full">
        <img
          className="w-[80px] h-[80px] rounded-full object-cover mr-4 shadow-lg"
          src={picture}
          alt="avatar"
        />
        <div className="flex-1 w-full">
          <div className="flex items-center justify-between">
            ...
            <button onClick={() => deletePost(id)}>
              <svg
                className="h-6 w-6 text-black hover:text-red-600"
                width="24"
                height="24"
                viewBox="0 0 24 24"
                strokeWidth="2"
          ...
          <p className="mt-2 text-gray-700 text-sm overflow-hidden text-ellipsis max-h-16 line-clamp-3">
            {content}
          </p>
          ...
          <div className="mt-4 flex justify-between items-center w-full">
            <button
              ...
              onClick={() => changeLikes(id, rank)}
            >
              {activeLike ? (
                <svg className="w-5 h-5 text-red-500 fill-current" viewBox="0 0 512 512">
            ...
            <div className="flex items-center text-gray-700 text-sm hover:text-red-400">
              <svg
              ...
              <button onClick={() => setDialog(true)}>Chat</button>
            </div>
          </div>
        </div>
      </div>
      {dialog && (
        <ResponseDialog setDialog={setDialog} title={title} content={content} postId={id} userName={userName} />
      )}
    </div>
  );
};
